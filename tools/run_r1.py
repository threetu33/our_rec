...
"""
    ...
    def evaluate_like_training(self, 
                             ...
                             save_interval: int = 50) -> Dict[str, float]:  # 新增：保存间隔
        """
        评估函数，完全模拟训练脚本中的评估过程
        ...
        if num_samples is None:
            ...
            print(f"\n=== Evaluating API like Training Script on {split} set ({num_samples}/{total_samples} samples) ===")
        ...
        for i in tqdm(range(min(num_samples, len(data))), desc=f"Evaluating {split}"):
            sample = data[i]
            ...
            # 构建可用商品集合：排除历史交互商品，与训练脚本的interacted_mask逻辑一致
            ...
            # 随机采样候选商品进行排序（模拟训练时的计算约束）
            sample_size = min(100, len(available_items))
            if target_id in available_items:
                # 确保目标商品在候选中
...
"""
            ...
            if save_results:
                detailed_results.append({
                    ...
                    'api_response': response[:200]  # 截断保存
                })
            ...
            if valid_samples % progress_interval == 0:
                ...
                print(f"\n--- Progress Update: {valid_samples}/{min(num_samples, len(data))} samples ---")
        ...
        if detailed_results:
            ...
            print(f"Average Predicted Items: {avg_predicted_items:.1f} (vs {len(candidate_items)} candidates)")
        ...
        if save_results:
            # 保存主要结果
            results_file = f"api_consistent_results_{split}_{self.model_name.replace('/', '_')}.json"
            ...
            if prompt_response_logs:
                debug_log_file = f"api_debug_logs_{split}_{self.model_name.replace('/', '_')}.json"
    ...
    def _save_incremental_results(self, accumulated_metrics, detailed_results, prompt_response_logs,
        ...
        try:
            ...
            # 主要结果的增量保存
            incremental_file = f"api_incremental_{split}_{self.model_name.replace('/', '_')}_{valid_samples}samples_{timestamp}.json"
            ...
            # 保存最新的调试日志（覆盖模式）
            if prompt_response_logs:
                debug_incremental_file = f"api_debug_incremental_{split}_{self.model_name.replace('/', '_')}.json"
    ...
    def run_full_evaluation(self, 
        ...
        test_metrics = self.evaluate_like_training(
            num_samples=test_samples,  # None表示全集
            split='test',
            progress_interval=progress_interval,  # 使用传入的进度间隔
            save_interval=save_interval  # 使用传入的保存间隔
        )
        ...
        if valid_metrics:
            ...
            summary_results['config']['valid_samples'] = valid_samples
        
        summary_file = f"api_consistent_summary_{self.model_name.replace('/', '_')}.json"
...
def main():
    ...
    parser.add_argument("--dataset_dir", type=str, default="/data/hongdeyao/Musical_Instruments_0_2022-10-2023-10", help="Dataset directory")
    parser.add_argument("--test_samples", type=int, default=None, help="Number of test samples (None for full dataset)")
