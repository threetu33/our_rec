...
- 为 prompts 生成 response
  ...
  - `output_file`：生成好reponse的文件保存地址
  - `prompt_id_max`：生成response的id上限
  - `prompt_id_min`：生成response的id下限

- 筛选正确的response并处理数据
  ```bash
  python rrec/sft_train/process_sft_data.py
  ```
  你可能需要修改的参数：
  - `input_file`：刚刚生成好reponse的文件地址
  - `output_file`：筛选好数据的文件保存地址

- sft训练流程(建议llama-factory)
...
```
你可能需要修改的参数：
- `BINARY_PROMPT_TMPL_MOVIES(train.py)`：如果你训练的是SFT模型，请修改为你SFT模型的prompt
- `BINARY_PROMPT_TMPL_OTHER(train.py)`：如果你训练的是SFT模型，请修改为你SFT模型的prompt
- `CUDA_VISIBLE_DEVICES`：使用的gpu
- `dataset_path`：数据集位置
- `dataset_type`：movies或者other
- `model_name_or_path`：待训练的模型位置
...
- **`preprocess.py`** - 数据预处理脚本
  ...
  - 若发现item数目不足10000，会向前3个月
  - 主要参数：
    - `--category` - 数据集类别
    - `--K` - 用户最少拥有多少交互历史(最多设置为20)
...
- **`generate_prompts.py`** - 生成二分类判断prompt
...
- **`deepseek.py`** - DeepSeek API评估
  ...
  - 运行示例：
    ```bash
    python run_deepseek_from_json.py \
      --test_samples 200 \
      --test_only \
      --candidate_mode json \
...
- **`train.py`** - PPO强化学习训练（Musical Instruments）
